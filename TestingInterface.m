...
sensor1 = [30,60]; sensor2 = [60,45]; sensorLocations = [sensor1 sensor2];
...
while runs <= maxRuns
    ...
    parfor j = 1:num_tests                                         % run sim for every UAS entrance location and record kill/nokill/NFZincursion in results vector                                           
        sim = simulator(map, AOR, UAS(15, entrances(j, :), asset1.location, 'Linear'), [effector1, effector2], [asset1], tps=20, animate=false, nfzs=NFZ1, resetGraphics=true, animationMultiplier=100);
...
for j = 1%:length(minCostID)
    ...
    parfor i = 1:height(entrances)                                                 % run sim for every UAS entrance location and record kill/nokill/NFZincursion in results vector                                           
            sim = simulator(map, AOR, UAS(15, entrances(i, :), asset1.location, 'Linear'), [effector1, effector2], [asset1], tps=20, animate=true, nfzs=NFZ1, resetGraphics=false, animationMultiplier=100);
